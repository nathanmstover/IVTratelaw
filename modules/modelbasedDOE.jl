...
function localMBDOEoptimization(model, params, initDPpoints, baseparams, noise; 
    upperDNA = 28.5, 
    lowerDNA = 1.0, 
    upperP = 384.0, 
    lowerP = 40.0, 
...
function multistartMBDOEoptimization(model, params, initDPpoints, baseparams, noise; 
    upperDNA = 28.5, 
    lowerDNA = 1.0, 
    upperP = 384.0, 
    lowerP = 40.0, 
    ...
    (minf,minx,ret) = localMBDOEoptimization(model, params, initDPpoints, baseparams, noise; upperDNA = upperDNA, lowerDNA = lowerDNA, upperP = upperP, lowerP = lowerP, kwargs...)

    for i in 1:npoints
        iterguess = [rand(Uniform(lowerbounds[j],upperbounds[j])) for j in 1:2*nvariables]
        (newminf,newminx,newret) = localMBDOEoptimization(model, params, iterguess, baseparams, noise; upperDNA = upperDNA, lowerDNA = lowerDNA, upperP = upperP, lowerP = lowerP, kwargs...)
